        ...
        >
            <div className={`bg-gradient-to-br ${panel.color} p-5 h-full min-h-[120px] flex flex-col justify-between`}>
                ...
                <div className="flex items-center gap-2 mb-3">
                    ...
                    {panel.tipo === "narrador" && (
                        <span className="text-xs font-bold text-white/50 uppercase tracking-wider italic"></span>
                    )}
                </div>
...
export default function UniversoPage() {
    ...
    const capsTraducidos = useMemo(() => {
        if (lang === 'espanol') return CAPÍTULOS;
        return CAPÍTULOS.map(cap => ({
            ...cap,
            titulo: dict.translate(cap.titulo),
            subtitulo: dict.translate(cap.subtitulo),
            fraseFinal: dict.translate(cap.fraseFinal),
            personaje: dict.translate(cap.personaje),
            viñetas: cap.viñetas.map(v => ({
                ...v,
                panels: v.panels.map(p => ({
                    ...p,
                    texto: dict.translate(p.texto),
                    personaje: 'personaje' in p && p.personaje ? dict.translate(p.personaje as string) : undefined,
                }))
            }))
        })) as typeof CAPÍTULOS;
    }, [lang, dict]);
        ...
        <main className="min-h-screen" style={{ background: "var(--navy)" }}>
            ...
            >
                ...
                >
                    ...
                    >
                        ⚡ {dict.translate("UNIVERSO CHISPITO — Las historias que cambian todo")}
                    </div>
                    <h1 className="font-fredoka text-5xl md:text-7xl text-white mb-4 leading-tight">
                        🚀 {dict.translate("La Historia de")}{" "}
                        <span style={{ color: "#FFD60A", textShadow: "0 0 30px #FFD60A80" }}>{dict.translate("Nico")}</span>
                    </h1>
                    <p className="text-white/60 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
                        {dict.translate("Desde su primer día en Kinder hasta graduarse de 3° Secundaria.")}<br />
                        <strong className="text-white/80">{dict.translate("10 grados. 10 amigos. Una historia que crece contigo.")}</strong>
                    </p>
                </motion.div>
